...
- Workers VPC (オンプレ <- `env.MESH.fetch()` - Workers)  
    ...
    キャッシュ動作はgrafanaと同様。ただし、OpenCode v2のBasic認証を肩代りし、Reasoningの通信をWorkersで落として通信量を削減。
    - **vscode**
    originの [Visual Studio Code Server](https://code.visualstudio.com/docs/remote/tunnels#_using-the-code-cli) (`code serve-web`) へHTTP接続。  
    キャッシュ動作はgrafanaと同様。ただし、接続トークンの入力`?tkn=`を省ける。
    - **linkding-mcp**  
